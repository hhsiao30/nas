...
from rl_project.nas.models.pg_controller import *
from params import *
...
def train(rank, args):
    ...
    for _iter in range(start_iter, NUM_ITERS):
        ...
        actions, probs, log_probs, h_out = controller_net.module.sample(h_in)
        print(actions)
        log_probs = torch.stack(log_probs).view(-1)
        print("Process {}: log_probs: {}".format(rank, log_probs))
        # initial state
        ...
        print(loss)
        print("Process {}: [Episode {}] loss: {}, reward: {}".format(rank, _iter, loss.item(), reward))
        ...
        print("Process {}: TNS: {}".format(rank, report["Setup TNS"]))
        print()
        optimizer.zero_grad()
        loss.backward()

        # tensorboard
        print("Process {}: tensorboard logging".format(rank))
        tb.add_scalar("loss", loss, _iter)
        tb.add_scalar("reward", reward, _iter)
        tb.add_scalar("TNS(ns)", report["Setup TNS"], _iter)
        tb.add_scalar("WNS(ns)", report["Setup WNS"], _iter)
        tb.add_scalar("Power(mW)", report["Total Power"], _iter)
        tb.add_scalar("net_length", report["net_length"], _iter)
        for name, weight in controller_net.named_parameters():
            if weight.grad.size(dim=0) >= 2:
                tb.add_histogram(name, weight, _iter)
                tb.add_histogram("{}.grad".format(name), weight.grad, _iter)
        tb.flush()
