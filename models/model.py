...
class PolicyNet(nn.Module):
    ...
    def forward(self, x, h_in, c_in, parameters):
        x = torch.unsqueeze(x, 0) # (length, batch, input_size)
        output, (h_out, c_out) = self.lstm(x, (h_in, c_in)) # output: (length, batch, hidden_size)
    ...
    def sample(self, state, h_in, c_in, parameters):
        action_dists, h_out, c_out = self.forward(state, h_in, c_in, parameters) # discrete: prob, conti: dist parameters
        actions = {}
        total_log_prob = 0
        ...
        return actions, total_log_prob, h_out, c_out