...
class ControllerNet(nn.Module):
    ...
    def forward(self, x, h_in, parameter):
        ...
        print({param: prob for param, prob in zip(parameters, probs)})
        print()
    ...
    def sample(self, h_in):
        ...
        for parameter in parameters:
            ...
            action = action.item()
            # action, log_prob, h_out = policy_net.module.sample(state, h_in, parameter)
        ...
        return actions, torch.stack(probs).view(-1), torch.stack(log_probs).view(-1), h_out